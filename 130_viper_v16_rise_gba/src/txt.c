...
{
	char buf[] = "ＨＰ　地底人：０００　アキラ：０００";

	buf[15] += SwiDiv(Txt.var1, 100);
	buf[17] += SwiDivMod(SwiDiv(Txt.var1, 10), 10);
	buf[19] += SwiDivMod(Txt.var1, 10);

	buf[31] += SwiDiv(Txt.var2, 100);
	buf[33] += SwiDivMod(SwiDiv(Txt.var2, 10), 10);
	buf[35] += SwiDivMod(Txt.var2, 10);

	Txt.x = 11;
	Txt.y = 2;

	SprSetGray();
	TxtDrawStr(buf, false);
}
//---------------------------------------------------------------------------
