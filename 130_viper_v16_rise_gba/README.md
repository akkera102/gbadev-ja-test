...
作りはGTSのものを流用してるのであまり代り映えしないです。

- 画質はリサイズ1/2 256色 mode4を採用しています。ガビガビです
- 縦スクロール操作中、アニメーションするように修正しました
- 容量に余裕があったので音声を10512Hzから16384Hzに変更しました
- 場面ジャンプのほかに、バトル中のみ自動勝ち負け機能を加えました
- 全ルートのプレイ時間は1時間程度です
...
セレクトボタンにてフラグ変更可能です。フラグと場面の組み合わせによってはバグが発生する為、値を強制変更しました。
