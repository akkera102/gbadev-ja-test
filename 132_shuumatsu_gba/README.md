...
対応バージョンと必要ファイルは以下のとおりです。「gbfs\data」フォルダに入れてください。一部のadpファイルは「終末DVD」フォルダにも重複して存在します。同じデータですのでどちらを使用してもかまいません。
```
・DVD版
...
- オプションの「画像、音楽、効果音」は内部変数を書き換えています。ストーリー進行不能にはなりませんが場面変化するまでそのままです
