...
対応バージョンと必要ファイルは以下のとおりです。「gbfs\data」フォルダに入れてください。一部のadpファイルは重複しており「終末DVD」フォルダにもあります。データは全て同じなのでどちらを使用してもかまいません。
```
・DVD版
...
- 無謀にもmode3アルファブレンドを使っているため、一部エフェクトでお見苦しい点もあるかと思います。モノを投げないでください
- オートセーブは実装していない為、手動保存した内容が最新扱いになります
- エンディング後にセーブせず電源を切ると既読フラグが消えます。「最初から」を選んでセーブしてください
- オプションの「画像、音楽、効果音」は変数を書き換えています。ストーリー進行不能にはなりませんが場面変化するまでそのままです
