...
対応バージョンと必要ファイルは以下のとおりです。「gbfs\data」フォルダに入れてください。一部のadpファイルは重複して別フォルダにも存在します。同じデータですのでどちらを使用してもかまいません。
```
・DVD版
...
無謀にもアルファブレンドエフェクトを使っているため、一部でお見苦しい点もあるかと思います。モノを投げないでください。

- 640x480サイズを4ドット削って636x480。GBA版は3で割って212x160にしています
- オートセーブ機能はありません。手動保存した内容が最新扱いになります
- エンディング後にセーブしないまま電源を切ると既読フラグが消えます。「最初から」を選んでセーブしてください
