...
変換作業にはPython3とPillowを使用します。別途インストールを行い、プロンプトのパスが通っていることを確認してください。Microsoft Visual C++ Redistributable(Visual Studio 2015, 2017, 2019, and 2022)もインストールをお願いします。「make.bat」を実行するとROMファイルが自動的に作られます。i5+SSD環境で変換時間は20-30分ほど。前編約29MB、後編約28MBのROMが作られれば成功です。ちなみにコンバート中にエラーが発生しても止まりません。やり直したい場合はDOSプロンプト画面を閉じて「gbfs\data」直下にいくつかある作業フォルダを削除してください。なお32bit環境パソコンには未対応です。
...
[hoshiさん](https://twitter.com/hoshi903)に作って頂きました。GB音源でFM音源を再現してもらう無茶ぶりに快く承諾して頂いて大変助かっています。ネタで作ってもらったGBA起動時の音には思わずニヤリとなるでしょう。落下音、ドスン音もお気に入りです。効果音のみのテストロムも用意していますので参考にされる方は[こちら](https://github.com/akkera102/gbadev-ja-test/tree/main/123_nisfx_gpsg)をご覧ください。

最後にゲームを制作されたフォア・ナインのスタッフ様にお礼を言わせてください。自分の中のマイベストゲームであり、エンディングを迎えた時の気持ちは生涯忘れることができません。ありがとうございます。
...
- 7章と21章に連続して同じ文章が出現します。会話フラグの仕様です。バグではありません。
- エミュの場合できればBIOSを使ってください。オープニングで微妙にズレます。
...
実機はバッテリーがあまり持たないので注意してください。GBA SPとEverDriveの場合、稼働時間は2時間程度と思われます。
...
- 重複文章の修正（cs05_20、cs08_18、cs17_26）
- 重複文章についてをREADMEに記載
- 選択肢の順番変更（cs07_09）
- タイトルの背景色を変更
