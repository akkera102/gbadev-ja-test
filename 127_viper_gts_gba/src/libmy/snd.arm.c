...
//	REG_SOUNDCNT_L = 0;
//	REG_SOUNDCNT_H = SNDA_RESET_FIFO | SNDA_VOL_100 | DSOUNDCTRL_ATIMER(0);
	*(vu8*)(REG_BASE + 0x83) = (SNDA_RESET_FIFO | SNDA_VOL_100 | DSOUNDCTRL_ATIMER(0)) >> 8;
...
//	REG_SOUNDCNT_H &= ~(SNDA_R_ENABLE | SNDA_L_ENABLE | SNDA_RESET_FIFO);
	*(vu8*)(REG_BASE + 0x83) = (~(SNDA_R_ENABLE | SNDA_L_ENABLE | SNDA_RESET_FIFO)) >> 8;

	REG_TM1CNT_H    = 0;
	REG_DMA1CNT     = 0;

//	REG_SOUNDCNT_H |=  (SNDA_RESET_FIFO);
	*(vu8*)(REG_BASE + 0x83) = (SNDA_RESET_FIFO) >> 8;
}
//---------------------------------------------------------------------------
