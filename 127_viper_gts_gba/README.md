...
- 対応バージョンは以下の通りとなっています。「SGS.DAT」のみを「gbfs\data」にコピーしてください。LIMITED EDITIONは未対応です。
```
...
- 基本1枚絵をパタパタアニメにしていますが、口パクなどは独立してアニメーションしています
- 恐らくオリジナルは24fps。なのにGBAは60fps。タイミングは無茶苦茶です！住む住民が違いました
...
フラグはメニュー表示時、スタートとセレクトボタンで変更可能です。メルセデスフラグは必須、カルラフラグは小話分岐となっています。
