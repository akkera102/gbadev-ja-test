...
このソフトはWindows版「ViperGTS」をGBAへ移植したものです。ゲームデータは付属していない為、製品を持っている方のみ遊べます。なおLIMITED EDITIONは未対応です。

![README](README.png)

## 前準備

- 対応バージョンは以下の通りとなっています。「SGS.DAT」のみを「gbfs\data」にコピーしてください。
```
・DLSite（ファイル容量30.12MB）　※将来変更の可能性あり 
・CDケース版（Win95）
...
劣化移植です、品質はお察しください（げぼば、ひでぶ。

- 容量増大に伴い、mode3（32768色）ではなくmode4（256色）を選んでいます
- 画像は単純リサイズをした為、劣化が激しいです。お腹も痛いです
- 基本1枚絵をパタパタアニメにしていますが、口パクなど独立してアニメーションさせています
- タイミングは無茶苦茶です。恐らくオリジナルは24fps。なのにGBAは60fps。住む住民が違いました
- レイアウトの都合、12x10フォントをやめて8x8フォントに変更しました
- システムメニューを簡略化した為、セーブデータは３個のみ。代わりに場面ジャンプを強化しました
...
スタートとセレクトボタンで変更可能です。メルセデスフラグは必須、カルラフラグは小話の分岐となっています。
